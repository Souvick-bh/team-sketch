...
export default function SignupPage() {
    ...
    const handleSubmit = async(e: React.FormEvent<HTMLFormElement>) => {
        ...
        if(form.username.trim() === "" || form.email.trim() === "" || form.password.trim() === "") {
            ...
            try {
                const signupResp = await axios.post(`${process.env.NEXT_PUBLIC_BASE_URL!}/api/user/signup`, form);
                if(signupResp.status === 200 || signupResp.status === 201) {
        ...
        <div className="flex flex-col flex-1 gap-8 items-center justify-center">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                <div className="flex px-5 py-1 gap-6 border rounded-sm border-white">
                    ...
                    <input className="text-center border-none focus:outline-none" type="text" name="username" value={form.username} onChange={handleChange} placeholder="unique-username"/>
                </div>
                ...
                <button className="text-center px-5 py-1 gap-6 border rounded-sm border-white" type="submit">Sign Up</button>
            </form>
        </div>
    );
}
